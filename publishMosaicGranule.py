...
##Geoserver_URL=string http://192.168.50.170:8080/geoserver
...
r = requests.post(url, jsonRunImporter, auth=('admin', 'geoserver'), headers=headers)
print(r.text)
...
r = requests.post(url, files=upload, auth=('admin', 'geoserver'))
...
print str(url)
r = requests.post(url, auth=('admin', 'geoserver'))
print "task started!"
